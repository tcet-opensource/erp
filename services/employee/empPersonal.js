...
export async function addNewEmployeePersonal(employeePersonalDetails, session) {
  const {
    uid,
    title,
    empFirstName,
...
export async function getEmployeePersonal(filter, limit, page) {
  const employeePersonal = await EmployeePersonal.read(filter, limit, page);
  if (employeePersonal) {
